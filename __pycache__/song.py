...
class songAPI:
    ...
    async def play(self, ctx,search: str):
        ...
        if voice_client == None:
            await ctx.channel.send("CiGrip Jointed")
            await channel.connect()
            voice_client = get(self.bot.voice_clients, guild=ctx.guild)
            
        if voice_client.channel != ctx.author.voice.channel:
            em = discord.Embed(title= "get in my toom before you command me, dumbass" , color= 0xcc0039)
    ...
    async def stop(self, ctx):
        voice_client = get(self.bot.voice_clients, guild=ctx.guild)
        if voice_client == None:
            em = discord.Embed(title= "I,PLEBS BOT,is not in any VC" , color= 0xcc0039)
            return await ctx.send(embed=em)
    ...
    async def leave(self, ctx):
        ...
        em = discord.Embed(title="SAYYONARA NIG**", color=0xcc0039)
    ...
    async def queueList(self, ctx):
        ...
        if player.queue.empty():
            em = discord.Embed(title= "There's no queue" , color= 0xcc0039)
            return await ctx.send(embed=em)
